...
test.describe('Fake Store API - Product Endpoint Tests', () => {
	...
	test('Should fetch product with 200 OK status and validate response structure', async ({ request }) => {
		...
		console.log(`[32m[1m✓ Response status: ${response.status()} OK[0m`);
		...
		requiredKeys.forEach(key => {
			...
			console.log(`[32m✓ Key '${key}' is present in response[0m`);
		});
		...
		if (!isValidSchema) {
			console.error(`[31m✗ Schema validation failed: ${JSON.stringify(ajv.errors)}[0m`);
			expect(isValidSchema).toBe(true); // This will fail the test with schema error details
		} else {
			console.log(`[32m✓ Response matches expected JSON schema[0m`);
	...
	test('Should handle invalid endpoint with appropriate error', async ({ request }) => {
		...
		expect(response.status()).toBe(200);
		console.log(`[32m✓ Response status: ${response.status()}[0m`);
